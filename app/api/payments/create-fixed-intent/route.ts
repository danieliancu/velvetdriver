...
import { createFixedPayNowIntent } from '@/lib/ride-payments';
...
export async function POST(request: Request) {
  try {
    ...
    const result = await createFixedPayNowIntent({
      amount,
    ...
    console.error('Create fixed payment intent error', err);
    return NextResponse.json({ error: err?.message || 'Failed to create fixed payment' }, { status: 500 });
  }
}
